...
var vrag1img = new Image();
vrag1img.src = 'images/ships/ship1.png';

var vrag2img = new Image();
...
fonimg.src = 'osnova.png';

//fonimg.getElementsByClassName('stile2.2');

window.onmousemove =function(event) {
    player.x = event.pageX*1.65;
};
...
//}
    ...
    for (j in fire1) {
        ...
        if (Math.abs(player.x + 35  - fire1[j].x - 35) < 75 && Math.abs(player.y - fire1[j].y+100) < 75) {
            //произошло столкновение
   ...
   // if (player.del == 1){player.splice();}
    ...
    for (j in fire2) {
        ...
        if (Math.abs(player.x + 35  - fire2[j].x - 35) < 75 && Math.abs(player.y - fire2[j].y+100) < 75) {
            //произошло столкновение
...
function render() {
    ...
    for (i in vrag1){context.drawImage(vrag1img,vrag1[i].x,vrag1[i].y,70,70)}
    for (i in vrag2){context.drawImage(vrag2img,vrag2[i].x,vrag2[i].y,140,140)}
